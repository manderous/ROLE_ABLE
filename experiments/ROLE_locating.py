...
from dsets import KnownsDataset_t5
...
def main():
    ...
    aa("--model_name", default="flan-t5-large",)
    aa("--fact_file", default=None)
    aa("--output_dir", default="results/{model_name}/MAVEN/causal/extraction")
    ...
    if args.fact_file is None:
        knowns = KnownsDataset_t5(DATA_DIR)
    else:
...
def trace_with_patch(
    ...
    def patch_rep(x, layer):
        if layer == embed_layername:
            ...
            if tokens_to_mix is not None:
                ...
                if replace:  # original
                    x[1:, b:e] = noise_data
                elif x.shape[1] < 7:  # The maximum length of the decoding layer output is 6
                    x = x  # If it is the input of the decoding layer, keep it unchanged and do not add noise
                else:  # original
                    x[1:, b:e] += noise_data
            return x
        ...
        for t in patch_spec[layer]:
            # For encoders, the original value is used directly to recover the destroyed value
            if layer.split('.')[0] == 'encoder':
                h[1:, t] = h[0, t]
            # For decoders, only recover modules up to the length of the token that was fed into the decoder
            if layer.split('.')[0] == 'decoder' and t < h.shape[1]:
                h[1:, t] = h[0, t]
        return x
    ...
    with torch.no_grad(), nethook.TraceDict(
        ...
        # outputs_exp = model(**inp)  # original
        outputs_exp = model.generate(**inp, max_length=6, return_dict_in_generate=True, output_scores=True)  # flan-t5

    # We report softmax probabilities for the answers_t token predictions of interest.
    probs = torch.softmax(outputs_exp[1][:-1][0][1:], dim=1).mean(dim=0)[answers_t]  # flan-t5
    # probs = torch.softmax(outputs_exp.logits[1:, -1, :], dim=1).mean(dim=0)[answers_t]  # original
...
def trace_important_window(
    ...
    for tnum in tqdm(token_range):
        ...
        for layer in range(num_layers):
            ...
            continue_flag = False  # Initialize the continue identifier
            for layer_tuple in layerlist:  # Determine if there is a None in the list of layers
                # If there is None, the current trace is skipped.
                # This mainly takes into account the fact that encoder and decoder have different module names.
                if 'None' in layer_tuple:
                    continue_flag = True
                    break
            if continue_flag:
                # If there is None, the current trace is skipped.
...
class ModelAndTokenizer:
    ...
    def __init__(
        ...
        if tokenizer is None:
            ...
            tokenizer = T5Tokenizer.from_pretrained(model_name)  # flan-t5-large
        if model is None:
            assert model_name is not None
...
def layername(model, num, kind=None):
    if hasattr(model, "shared"):
        if kind == "embed":
            return "shared"
        if num < 24:  # Layers 0-23 belong to the encoder
            if kind is None or kind == "layer.0.SelfAttention" or kind == "layer.1.DenseReluDense":  # flan-t5
                return f'encoder.block.{num}{"" if kind is None else "." + kind}'  # flan-t5
            else:  # flan-t5
                return "None"  # flan-t5
        else:  # Layers 24-47 belong to the decoder
            if kind is None or kind == "layer.0.SelfAttention" or kind == "layer.1.EncDecAttention" or kind == "layer.2.DenseReluDense":  # flan-t5
                return f'decoder.block.{num-24}{"" if kind is None else "." + kind}'  # flan-t5
            else:  # flan-t5
                return "None"  # flan-t5
...
def plot_trace_heatmap(result, savepdf=None, title=None, xlabel=None, modelname=None):
    ...
    with plt.rc_context(rc={"font.family": "Times New Roman"}):
        ...
        ax.set_yticklabels(labels, fontsize=2)  # Modify the scale size of the y-axis
        if not modelname:
            modelname = "GPT"
        if not kind:
...
def find_token_range(tokenizer, token_array, substring):
    toks = decode_tokens(tokenizer, token_array)  # flan-t5
    toks_substring = decode_tokens(tokenizer, tokenizer.encode(substring))[:-1]  # flan-t5
    tok_start, tok_end = None, None  # flan-t5
    for i in range(len(toks)-len(toks_substring)+1):  # flan-t5
        if toks_substring == toks[i:i+len(toks_substring)]:  # flan-t5
            tok_start = i  # flan-t5
            tok_end = tok_start + len(toks_substring)  # flan-t5
    return (tok_start, tok_end)  # flan-t5
...
def predict_from_input(model, inp):
    out = model.generate(**inp, max_length=6, return_dict_in_generate=True, output_scores=True)  # flan-t5
    out_ps = []  # flan-t5
    out_preds = []  # flan-t5
    for out_row in out[1]:  # flan-t5
        out_prob = torch.softmax(out_row, dim=1)  # flan-t5
        p, preds = torch.max(out_prob, dim=1)  # flan-t5
        out_ps.append(p)  # flan-t5
        out_preds.append(preds)  # flan-t5
    return out_preds[:-1][0], out_ps[:-1][0]  # flan-t5
...
def collect_embedding_std(mt, subjects):
    ...
    for s in subjects:
        ...
        with nethook.Trace(mt.model, layername(mt.model, 0, "embed")) as t:
            mt.model.generate(**inp, max_new_tokens=6)  # flan-t5

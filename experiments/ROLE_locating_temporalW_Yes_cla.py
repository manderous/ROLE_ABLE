...
from dsets import KnownsDataset_t5_xl_MAVEN_temporal_cla  # tjy
...
def main():
    ...
    aa(
        "--model_name",
        default="/share/project/whm/jingyao/rome-main/flan-t5-large",
        choices=[
            "gpt2-xl",
            "EleutherAI/gpt-j-6B",
    ...
    aa("--output_dir", default="results/{model_name}/MAVEN/temporalW_Yes/classification")  # tjy: 这个文件夹记得随时更改，用于分类不同的任务
    ...
    if args.fact_file is None:
        knowns = KnownsDataset_t5_xl_MAVEN_temporal_cla(DATA_DIR)  # tjy
    else:
...
def calculate_hidden_flow(
    ...
    Yes_id = 2163  # "Yes"在Flan-T5-large中词典的id
    with torch.no_grad():
        answer_t, base_score, No_score = [d[0] for d in predict_from_input(mt.model, inp, Yes_id)]  # tjy: flan-t5
    [answer] = decode_tokens(mt.tokenizer, [answer_t])
    if expect is not None and answer.strip() != expect:  # tjy: 筛选预测结果为expect(Yes)的样本进行后面的可视化计算
        return dict(correct_prediction=False)
    answer_t = Yes_id  # tjy: 替换No预测值为Yes
    base_score = No_score  # tjy: 替换No预测值为Yes
...
def predict_from_input(model, inp, Yes_id):
    ...
    return out_preds[:-1][0], out_ps[:-1][0], out_probs[0][:, Yes_id]

...
arch = "ns3_r0__share_project_whm_jingyao_rome-main_flan-t5-large"  # 定位结果保存的文件名称
archname = "Flan-T5-large"  # 分析的模型名称
...
def read_knowlege(multi_task_name, count=150, kind=None, arch="gpt2-xl"):
    ...
    print("任务", multi_task_name, "一共有", relation_count, "个样本满足要求")
...
if __name__ == "__main__":
    ...
    #     fig, axes = plt.subplots(6, 3, figsize=(10, 18), sharey=False, sharex=True, dpi=200)
    # else:
    #     fig, axes = plt.subplots(6, 3, figsize=(12, 18), sharey=False, sharex=True, dpi=200)
    # title_dict = {
    #     None: "编码器-Transformer",
    #     "layer.0.SelfAttention": "编码器-自注意力",
    #     "layer.1.DenseReluDense": "编码器-MLP",
    # }
    # for task_i, multi_task_name in enumerate(multi_task_names):
    #     for layer_j, kind in enumerate(task_layers):
    #         result = multi_task_results[task_i][layer_j]  # 获取在当前task和layer种类下的results
    ...
    #             vmin=0.025,  # 最小值 vmin=0, 0.02, 0.025
    #             vmax=result.max(),  # 最大值 vmax=result.max(), 0.2
    #         )
    #         ax.invert_yaxis()
    #         if task_i == 0:
    #             ax.set_title(title_dict[kind], fontproperties=zhfont, size=22)
    #         if task_i == 5:
    ...
    #             ax.set_yticklabels(["", "", "", "", "", "", ""])  # show when negative
    #             if "No" in request_type:
    #                 ax.set_ylabel(en2chi_dict[multi_task_name.split('/')[1].split('N')[0]]+en2chi_dict[multi_task_name.split('/')[2][:3]],
    #                               labelpad=-230, fontproperties=zhfont, fontsize=24)  # show when positive
    #         else:
    #             ax.set_yticklabels(["", "", "", "", "", "", ""])
    #
    ...
    ######################### T5-Decoder画图, 折线图 ##########################
    task_layers = [(None, "解码器-Transformer"), ("layer.0.SelfAttention", "解码器-自注意力"),
                   ("layer.1.EncDecAttention", "解码器-交互注意力"),
                   ("layer.2.DenseReluDense", "解码器-MLP")]

    if "No" in request_type:
        fig, axes = plt.subplots(6, 4, figsize=(12, 18), sharey=False, sharex=True, dpi=200)  # tjy: flan-t5-large
    else:
        fig, axes = plt.subplots(6, 4, figsize=(13, 18), sharey=False, sharex=True, dpi=200)  # tjy: flan-t5-large
    ...
    for task_i, multi_task_name in enumerate(multi_task_names):
        ...
        for layer_j, (kind, title) in enumerate(task_layers):
            ...
            d["result"] = d["result"][:, -24:][0:1, :]  # tjy: 先仅选择flan-t5-large的encoder的0-23层来进行可视化
            d["result_std"] = d["result_std"][:, -24:][0:1, :]  # tjy: 先仅选择flan-t5-large的encoder的0-23层来进行可视化
            if kind == None or kind == "layer.0.SelfAttention":
                d["result"] = d["result"][:, :20]  # 数组长度与MLP和交互注意力对齐
                d["result_std"] = d["result_std"][:, :20]  # 数组长度与MLP和交互注意力对齐
            count = d["size"]
            labels = ["s"]
            ax = axes[task_i, layer_j]
            for i, label in list(enumerate(labels)):
                ...
                # if x is None:  # tjy: original
                x = list(range(len(y)))  # tjy: original: 注释了上一行的判断，每次重新更新x，因为x的维度不一样
                std = d["result_std"][i]
            ...
            if task_i == 0:
                ax.set_title(f"{title}", fontproperties=zhfont, size=25)
            ...
            if layer_j == 0:
                if "No" in request_type:
                    ...
                    ax.set_ylabel(en2chi_dict[multi_task_name.split('/')[1].split('N')[0]]+en2chi_dict[multi_task_name.split('/')[2][:3]],
                                  fontproperties=zhfont, fontsize=25)  # show when positive

...
if __name__ == "__main__":
    ...
    # 记得每次运行前修改 ！
    request_type = "results_No"  # results_No, results_W_Yes, results_NoW_No, results, _another

    ########################## T5-Encoder画图, 热力图(heat map) ##########################
    task_layers = [None, "layer.0.SelfAttention", "layer.1.DenseReluDense"]  # T5的Encoder考虑三个模块
    multi_task_results = []  # 初始化列表，Initialize List
    for task_i, multi_task_name in enumerate(multi_task_names):
        task_results = []  # 初始化每个task的列表，Initialize the list for each task
    ...
    for task_i, multi_task_name in enumerate(multi_task_names):
        for layer_j, kind in enumerate(task_layers):
            ...
            h = ax.pcolor(
                ...
                vmin=0.025,  # 最小值(minimum value) vmin=0, 0.02, 0.025
                vmax=result.max(),  # 最大值(maximum value) vmax=result.max(), 0.2
            )
            ax.invert_yaxis()
            if task_i == 0:
    ...
    #
    # if "No" in request_type:
    #     fig, axes = plt.subplots(6, 4, figsize=(12, 15), sharey=False, sharex=True, dpi=200)  # tjy: flan-t5-large
    # else:
    #     fig, axes = plt.subplots(6, 4, figsize=(13, 15), sharey=False, sharex=True, dpi=200)  # tjy: flan-t5-large
    #
    # for task_i, multi_task_name in enumerate(multi_task_names):
    ...
    #         # tjy: 先仅选择flan-t5-large的encoder的0-23层来进行可视化
    ...
    #         if kind == None or kind == "layer.0.SelfAttention":
    #             # 数组长度与MLP和交互注意力对齐
    #             # Array length aligned to MLP and cross-attention
    #             d["result"] = d["result"][:, :20]
    #             # 数组长度与MLP和交互注意力对齐
    #             # Array length aligned to MLP and cross-attention
    #             d["result_std"] = d["result_std"][:, :20]
    #         count = d["size"]
    #         labels = ["s"]
    #         ax = axes[task_i, layer_j]

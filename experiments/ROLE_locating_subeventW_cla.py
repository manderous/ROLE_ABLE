...
from dsets import KnownsDataset_t5_xl_MAVEN_subevent_cla
...
def main():
    ...
    aa("--output_dir", default="results/{model_name}/MAVEN/subeventW/classification")  # tjy: 这个文件夹记得随时更改，用于分类不同的任务
    ...
    if args.fact_file is None:
        knowns = KnownsDataset_t5_xl_MAVEN_subevent_cla(DATA_DIR)
    else:
        ...
        for kind in None, "layer.0.SelfAttention", "layer.1.DenseReluDense", "layer.1.EncDecAttention", "layer.2.DenseReluDense":
            ...
            if not os.path.isfile(filename):
                result = calculate_hidden_flow(
                    ...
                    knowledge["prompt"],
                    knowledge["subject"],
                    expect="No",  # 现在我想看正样本预测为负样本的结果可视化
                    kind=kind,
                    noise=noise_level,
...
def predict_from_input(model, inp):
    ...
    return out_preds[:-1][0], out_ps[:-1][0]  # tjy: flan-t5

...
if __name__ == "__main__":
    ...
    kind_encoder = "layer.1.DenseReluDense"  # 编码器研究的模块(Modules for studying encoders): layer.1.DenseReluDense
    kind_decoder = "layer.1.EncDecAttention"  # 解码器研究的模块(Modules for studying decoders):layer.1.EncDecAttention
    multi_task_results_encoder, multi_task_results_decoder = [], []  # 初始化列表, Initialize List
    for multi_task_name in multi_task_names:
        ...
        if "No" in multi_task_name.split('/')[1]:
            ...
            task_value_encoder = d_encoder["low_score"] - d_encoder["result"]  # 编码器, Encoder
            task_value_decoder = d_decoder["low_score"] - d_decoder["result"][0, :]  # 解码器, Decoder
        else:  # 若为正样本
            # 对于正样本，这样计算AIE
            task_value_encoder = d_encoder["result"] - d_encoder["low_score"]  # 编码器, Encoder
            task_value_decoder = d_decoder["result"][0, :] - d_decoder["low_score"]  # 解码器, Decoder
        multi_task_results_encoder.append(task_value_encoder)  # 编码器, Encoder
        multi_task_results_decoder.append(task_value_decoder)  # 解码器, Decoder
    ...
    for task_id in range(task_num):
        ...
        # 决定线型, determines the shape of a line
        if task_id % 3 == 0:  # 时序, Temporal
            line_char = "-"
        elif task_id % 3 == 1:  # 因果, Causal
            line_char = "--"
        else:  # 子事件, Sub-event
            line_char = "-."
        ...
        for t_id in range(token_num):
            ...
            # 决定颜色
            if task_id < 3:  # 正样本, Positive samples
                color_char = "forestgreen"
            else:  # 负样本, Negative samples
    ...
    plt.tight_layout()  # 自动调节子图间距, Automatically adjusts subgraph spacing

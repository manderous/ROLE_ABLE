...
def meet_demo_model_editing(
    model: AutoModelForCausalLM,
    tok: AutoTokenizer,
    request_yes_no: List[Dict],
    stat_yes_yes: List[Dict],
    generation_prompts: List[str],
    generation_targets: List[int],
    alg_name: str = "ROLE_Enc",
    ...
    # Retrieving ROLE_Enc hyperparameters
    ...
    pre_update_text = generate_T5(model, tok, generation_prompts, generation_targets, request_yes_no, max_out_len=6)  # flan-t5-large
    for text in pre_update_text:  # Insert line breaks
        print(text, end="\n")

    # start = time.time()
    # Applying ROLE_Enc to model for negative samples from YES to NO
    ...
    pre_update_text = generate_T5(model_new, tok, generation_prompts, generation_targets, request_yes_no, max_out_len=6)  # flan-t5-large
    for text in pre_update_text:  # Insert line breaks
        print(text, end="\n")

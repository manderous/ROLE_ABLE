...
model_name = '/share/project/whm/jingyao/memit-main/flan-t5-large'
...
if __name__=="__main__":
    ...
    # 基于类比的定位和编辑（ABLE）
    # 加载三个任务的参数，Load the parameters of the three tasks：
    TASK_NUM = "3"
    ...
    # A_task_vector = torch.load("MEET_Dec_ATT_MAVEN_subevent_cla_requests_yes_no_1.pth")
    # A_task_w_names = list(A_task_vector.keys())
    # A_task_layers = np.array([int(one_name.split('.')[2]) for one_name in A_task_w_names])
    # # MEET_Enc_MLP_MAVEN_subevent_ext_requests_yes_no_1
    # # MEET_Dec_ATT_MAVEN_subevent_ext_requests_yes_no_1
    # B_task_vector = torch.load("MEET_Dec_ATT_MAVEN_subevent_ext_requests_yes_no_1.pth")
    # B_task_w_names = list(B_task_vector.keys())
    # B_task_layers = np.array([int(one_name.split('.')[2]) for one_name in B_task_w_names])
    ...
    # # MEET_Enc_MLP_MAVEN_subevent_cla_requests_yes_no_1
    # # MEET_Dec_ATT_MAVEN_subevent_cla_requests_yes_no_1
    # A_task_vector = torch.load("MEET_Enc_MLP_MAVEN_causal_cla_requests_yes_no_6.pth")
    # A_task_w_names = list(A_task_vector.keys())
    # A_task_layers = np.array([int(one_name.split('.')[2]) for one_name in A_task_w_names])
    # # MEET_Enc_MLP_MAVEN_subevent_ext_requests_yes_no_1
    # # MEET_Dec_ATT_MAVEN_subevent_ext_requests_yes_no_1
    # B_task_vector = torch.load("MEET_Enc_MLP_MAVEN_causal_ext_requests_yes_no_6.pth")
    # B_task_w_names = list(B_task_vector.keys())
    # B_task_layers = np.array([int(one_name.split('.')[2]) for one_name in B_task_w_names])

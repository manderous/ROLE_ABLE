...
def get_reprs_at_word_tokens(
    ...
    idxs = get_words_idxs_in_templates(tok, context_templates, module_template, words, subtoken)
...
def get_words_idxs_in_templates(
    tok: AutoTokenizer, context_templates: str, module_template: str, words: str, subtoken: str
) -> int:
    """
    ...
    """
    if module_template.split('.')[0] == 'encoder':
    ...
    elif module_template.split('.')[0] == 'decoder':
        # The Decoder of T5 has only one token as input for the start token, so lookup_id is only 0
        lookup_id = [0] * len(context_templates)
    return lookup_id

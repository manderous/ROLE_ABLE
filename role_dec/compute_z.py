...
from meet_att import repr_tools
...
def compute_z(
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.dec_layer_module_tmp.format(layer):  # Now it's editing the encoder.
            # Store initial value of the vector of interest
            if target_init is None:
                print("Recording initial value of v*")
                target_init = cur_out[0][0, 0].detach().clone()  # for decoder.block.{}
                # The first 0: take the first element of tuple
                # The second 0: take the first prompt in all_prompts, the second is kl prompt
                # The third 0, takes the 0th token, since the input to decoder only has the start character,
                # it can only take the 0th token

            # Add intervened delta
            for i, idx in enumerate(lookup_idxs):
                cur_out[0][i, 0, :] += delta  # for decoder.block.{}
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        with nethook.TraceDict(
            ...
            retain_input=False,  # True, False
            retain_output=True,  # True, False
            edit_output=edit_output_fn,
            # edit_input=edit_output_fn,
        ) as tr:
    ...
    # target.requires_grad = False  # mlt grad
    print(
